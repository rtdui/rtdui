...
import { getLeafColumns, getColumnDefId, pinyinComparer } from "../utils";
...
import { getPinColumnId } from "../utils/columnDefsUtils";
import { sortFn_pinyin } from "../fns/sortFns";
import { filterFn_arrHas, filterFn_arrNot } from "../fns/filterFns";
...
export function DataTable(props: DataTableProps) {
  ...
  const tableOptions: TableOptions<any> = {
    ...
    sortingFns: {
      pinyin: sortFn_pinyin,
    },
    filterFns: {
      arrHas: filterFn_arrHas,
      arrNot: filterFn_arrNot,
    },
  ...
  const [initPinToLeft, initPinToRight] = getPinColumnId(columns);

  const getInitFinalLeftPinning = () => {
    const result: string[] = [];
    if (enableRowSelection && !enableClickRowSelection) {
      result.push("选择");
    }
    if (enableAutoRowNumber) {
      result.push("行号");
    ...
    if (enableTree) {
      const firstExpandableId = flattenBy(
        columnsProp,
        (item: any) => item.columns,
      ).find((d) => d.meta?.expandable)?.id;
      if (firstExpandableId) {
        result.push(firstExpandableId);
      }
    }
    // 分组列必须在其它固定列之后. 因为在有列头组的情况下数据分组的列头的列头组的getPinnedIndex()为-1, 从而导致getPinnedSize()方法计算时无法得知其位置.
    result.push(...initPinToLeft);
    return result;
  };

  //#region 根据情况预设选项中的初始状态
  // 初始列序按定义顺序列出
  if (!tableOptions.initialState?.columnOrder?.length) {
    tableOptions.initialState ??= {};
    tableOptions.initialState.columnOrder = getLeafColumns(columnsProp).map(
      (d) => getColumnDefId(d) as string,
    );
  }

  // 初始pinned列
  if (!tableOptions.initialState.columnPinning?.left?.length) {
    tableOptions.initialState.columnPinning ??= {};
    // 按 `[复选框列]/[行号列]/[树展开列]/自定固定列` 顺序(如果有那种列的话)
    tableOptions.initialState.columnPinning.left = getInitFinalLeftPinning();
  }
  if (!tableOptions.initialState.columnPinning.right?.length) {
    tableOptions.initialState.columnPinning.right = initPinToRight;
  }
  //#endregion 根据情况预设选项中的初始状态
  ...
  //#region 行虚拟化
  const estimateRowHeight = () => (size === "sm" ? 36 : 56);
    ...
    >
      ...
      >
        {showToolbar && (
          <div
            className={clsx(
              "data-table-toolbar navbar flex items-center bg-base-300 relative z-10 py-0 min-h-0 text-xs",
              slots?.toolbar,
            )}
          >
            ...
            {enableExport && (
              <ExportTable table={table} tableRef={tableRef as any} />
            )}
        ...
        <div
          ...
          className={clsx(
            "data-table-container bg-base-100 flex-1 overflow-auto relative z-0",
            slots?.container,
          )}
          style={
            {
          ...
          <table
            ...
            className={clsx(
              ...
              // "relative", // .table 默认就是
              {
                ...
                "border-x border-base-200": showBorder,
                "table-xs": size === "xs",
                "table-sm": size === "sm",
                "table-md": size === "md",
                "table-lg": size === "lg",
          ...
          >
            ...
            <thead
              ...
              className={clsx("bg-base-200", {
                "sticky top-0 z-2": enableStickyHeader,
              })}
            >
                ...
                <tr key={headerGroup.id}>
                  {headerGroup.headers.map((header) => (
                    <HeaderCell
                      ...
                      initPins={[getInitFinalLeftPinning(), initPinToRight]}
                    />
                  ))}
                </tr>
              ))}
              ...
              {table.getRowModel().rows.length === 0 && (
                <tr>
                  <td
                    ...
                    className="text-center bg-base-100"
                  >
                    无数据
                  </td>
                </tr>
            ...
            {table
              ...
              <tfoot className={clsx("bg-base-200", "sticky bottom-0 z-1")}>
                {table
                  .getFooterGroups()
                  .slice(0, 1)
                  .map((footerGroup) => (
